...
async function loadBundledAudioManifest() {
    try {
        ...
        // Lecture des valeurs par défaut depuis le manifest (ou fallback)
        const defaultInhale = data.defaultInhale || 'cloche';
        const defaultExhale = data.defaultExhale || 'bol';
        const defaultMusic = data.defaultMusic || null;
...
document.getElementById('musicLibrary').style.display = 'block';
            ...
            if (musicSelect && defaultMusic && data.music.includes(defaultMusic)) {
                musicSelect.value = defaultMusic;
                // Sélectionner aussi dans musicLibrary pour préparer la lecture
                const idx = musicLibrary.findIndex(m => m.name === defaultMusic);
                if (idx >= 0) {
                    selectMusic(idx);
                }
            }
        ...
        function restoreOrSelectDefault(selectEl, storageKey, baseName, prefix) {
            ...
            // 1) Si une valeur sauvegardée existe ET correspond à une option valide
            if (saved && saved !== 'none') {
                ...
                // Sinon, on ignore la valeur sauvegardée obsolète et on continue vers le défaut
            }

            // 2) On force le défaut du manifest : m4a en priorité, puis mp3
            ...
            // 3) Dernier recours : sélectionner la première option disponible (autre que "none")
            const firstValidOpt = selectEl.querySelector('option:not([value="none"])');
            if (firstValidOpt) {
                selectEl.value = firstValidOpt.value;
                try { localStorage.setItem(storageKey, firstValidOpt.value); } catch (e) {}
                return firstValidOpt.value;
            }

            // 4) Vraiment aucun son disponible
            selectEl.value = 'none';
